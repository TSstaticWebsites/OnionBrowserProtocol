...
const DIRECTORY_PROXY_URL = import.meta.env.VITE_DIRECTORY_AUTHORITY_URL || 'http://localhost:3001';

export const nodeService = {
  async getAvailableNodes(): Promise<TorNode[]> {
    try {
      const response = await axios.get(`${DIRECTORY_PROXY_URL}/nodes`);
      // Parse the response which includes nodes in a nested structure
