...
export const OnionBrowser: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send to entry proxy
      const html = await proxyService.sendOnionRequest(packages, selectedCircuit.entryNode.address);
      setContent(html);
    } catch (error) {
        ...
        <CardContent>
          ...
          {circuit && (
            <div className="mt-4 space-y-2">
              ...
              <div className="text-sm space-y-1">
                <div>Entry: {circuit.entryNode.address}</div>
                <div>Middle: {circuit.middleNode.address}</div>
                <div>Exit: {circuit.exitNode.address}</div>
              </div>
            </div>
          )}
          ...
          {content && (
            <div className="mt-4">
              ...
              <div
                className="bg-secondary p-4 rounded-md overflow-auto max-h-96"
